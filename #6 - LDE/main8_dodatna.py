...
def f( x, t, k=0.1, T_out=-5.0, A=1.0, d=10.0 ):
    #return -k*(x - T_out)
    return -k*(x - T_out) + A*np.sin((2.0*np.pi/24.0)*(t - d))

def analyt( t, x0, k=0.1, T_out=-5.0 ):
    #return  T_out + np.exp(-k*t)*(x0 - T_out)
    values, _ = odeint(f, x0, t, full_output=1)
    return values[:, 0]
...
ax1.plot(t, x_analyt, ls='--', c='blue', alpha=0.8, lw=0.8, label=f'T_0 = {x0}')

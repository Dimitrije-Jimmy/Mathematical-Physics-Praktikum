...
#from tensorflow import linalg as TF
...
def diagonalize(matrix, tol=10**-10, maxiter=500):
    tridiag = matrix
...
def worker(n_sez, lamb):
    ...
    for matrix_size in n_sez:
        ...
        # LA.eig _______________________________________________________
        ...
        # Diagonalize __________________________________________________
        start_time = time.time()
        ...
        # Jacobi _______________________________________________________
        start_time = time.time()
        ...
        # QR numpy _____________________________________________________
        start_time = time.time()
        ...
        # LA.eigh ______________________________________________________
        start_time = time.time()
        ...
        # TF.eig _______________________________________________________
        start_time = time.time()
        data = data.tolist()
        data = tf.convert_to_tensor(data)
        complex_values, _ = tf.linalg.eigs(data)
        eigenvalues = np.real(complex_values)
        #print(eigenvalues)
        execution_time = time.time() - start_time  
        time_list.append(execution_time)
...
list_of_functions = [LA.eig, diagonalize, jacobi_eigenvalue, qr_algorithm_numpy, LA.eigh, TF.eig]
n_sez = [1] + np.arange(5, 100, 5)
lamb = 0.5
# call threads
...
functions = ['eig - Numpy', 'Householder', 'Jacobi', 'QR - Numpy', 'eigh - Numpy', 'eig - Tensorflow']
cmap = plt.get_cmap('viridis')
barve = [cmap(value) for value in np.linspace(0, 1, 6)]
...
plt.scatter(n_sez, sez_casov[:, 5], label=f'{functions[5]}', color=barve[5])
plt.plot(n_sez, sez_casov[:, 5], alpha=0.5, lw=0.3, color=barve[5])

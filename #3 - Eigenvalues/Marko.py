...
def wonky_exit(matrix, epsilon=10**-10):
    #print( np.abs(np.sum(np.abs(matrix)) - np.sum(np.abs(np.diag(matrix)))) < epsilon )
...
def diagonalize(matrix, tol=10**-15, maxiter=10000):
    ...
    for i in range(maxiter):
        ...
        #if np.abs(np.sum(np.abs(matrix)) - np.sum(np.abs(np.diag(matrix)))) < 10**(-10):
        
            print(i)
            break
        Q, R = qr_decomp(tridiag)
        tridiag = zero_filter(np.matmul(Q.T, np.matmul(tridiag, Q)), tol)
        #tridiag[np.abs(np.matmul(Q.T, np.matmul(tridiag, Q))) < tol] = 0.0
        s = np.matmul(s, Q)
...
lam = 0.5
data = anharmonic(lam, 50, q_matrix_single)
